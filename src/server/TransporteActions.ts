...
export const updateTransporte = createServerAction()
	...
	.handler(async ({ input }) => {
		...
		const isDataEqual =
			JSON.stringify(data?.emissao_cte) ===
			JSON.stringify(input.emissao_cte);

		console.log({ isDataEqual });
		...
		if (Object.keys(camposAlterados).length === 0) {
			return { message: "Nenhuma alteração foi realizada" };
		}

		// return { transporte, input, camposAlterados };
